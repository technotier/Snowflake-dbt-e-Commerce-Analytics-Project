...
-- create customer analytics dashboard view
create or replace view customers_analytics_view as 
with
...
from 
analytics_schema.fact_sales fs left join analytics_schema.dim_customers dc
on fs.customer_id = dc.id
group by 1, 2, 3, 4
),
customer_rfm_cte as (
select
